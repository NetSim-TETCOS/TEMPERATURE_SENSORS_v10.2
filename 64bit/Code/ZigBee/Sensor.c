...
int fn_NetSim_Zigbee_SensorEvent(int nSensorLoop,NETSIM_ID nGlobalPANCoordinatorId,AGENT** pstruAgent,METRICS** pstruMetrics,NetSim_EVENTDETAILS* pstruEventDetails)
{
	...
	char str[500];
	int nAgentLoop;
	POS_2D* pstruPos = NULL;
	double dDistance;
	POS_2D* pstruTemppos = NULL;
	NetSim_PACKET *PstruPacket;
	double sensed_temperature=0.0;
	...
	/*for(nAgentLoop =0;nAgentLoop<MAXAGENT;nAgentLoop++)
	{
		...
		if(dDistance <= pstru_Sensor[nSensorLoop]->dSensorRange)
		{
			...
			sensed_temperature=fn_NetSim_Temperature_Sensor_run(nSensorLoop);

			PstruPacket = fn_NetSim_Packet_CreatePacket(5);
			sprintf(PstruPacket->szPacketType,"SensorApp-%d",nPacketId);

			sprintf(str,"%lf",sensed_temperature);
			PstruPacket->szOverhead = _strdup(str);

			PstruPacket->dEventTime = pstruEventDetails->dEventTime;
			add_dest_to_packet(PstruPacket, nGlobalPANCoordinatorId);
			...
			PstruPacket->pstruNetworkData->szDestIP = IP_COPY(fn_NetSim_Stack_GetFirstIPAddressAsId(get_first_dest_from_packet(PstruPacket),0));
			...
			fn_NetSim_Add_DummyPayload(PstruPacket,str);
