									  ...
									  NETSIM_ID rx, NETSIM_ID ri)
{
	double p = propagation_get_received_power_dbm(propagationHandle,
												  ...
												  rx, ri,ldEventTime);
	assert(tri == 1);
	double rs = WSN_PHY(trx)->dReceiverSensivity;
	...
	case MAC_IN_EVENT:
		{
			...
			if(pstruPacket->nControlDataType/100 != MAC_PROTOCOL_IEEE802_15_4)
			{
				...
				if(pstruPacket->pstruAppData)
				{
					...
					pstruEventDetails->nApplicationId=pstruPacket->pstruAppData->nApplicationId;
					fn_NetSim_Temperature_Sensor_Update_Sink_Temp(pstruPacket->nSourceId,atof((const char*)pstruPacket->szPayload->packet));
	...
	case TIMER_EVENT:
		{
			...
			case SENSOR_EVENT:
				{
					ZIGBEE_SENSOREVENT(pstruEventDetails->nDeviceId);
				}
				break;
			...
			case MATLAB_EVENT:
				{
					fn_netsim_matlab_run(1,"","","",NETWORK->nDeviceCount,0,0,0);
...
{
	fprintf(stderr,"\n\nClosing MATLAB Engine Process\nPress any key....\n");
	_getch();
	fn_netsim_matlab_finish();
	fn_NetSim_Temperature_Sensor_finish();
...
/** This function is used to define the power model in wireless sensor networks. */
_declspec(dllexport) int fn_NetSim_Zigbee_PowerModel()
{
	NETSIM_ID nLoop = 0;
	if(pstruDevicePower)
	{
		ANIM_HANDLE handle = anim_add_new_menu(NULL,
											   "Battery Power",
											   false,
											   false,
											   false,
		...
		for(nLoop = 0;nLoop<NETWORK->nDeviceCount;nLoop++)
		{
			if(pstruDevicePower[nLoop] != NULL)
			{
				pstruDevicePower[nLoop]->dRemainingPower = pstruDevicePower[nLoop]->dInitialEnergy_mW;
				pstruDevicePower[nLoop]->nNodeStatus = IDLE;
				char name[BUFSIZ];
				sprintf(name, "%s_BatteryPower", DEVICE_NAME(nLoop + 1));
				pstruDevicePower[nLoop]->animHandle =
